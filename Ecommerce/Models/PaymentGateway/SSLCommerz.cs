...
using System.Net;
using System.Text;
namespace Ecommerce.Models.Web.PaymentGateway
{
        ...
        public string InitiateTransaction(NameValueCollection PostData, bool GetGateWayList = false)
        {
            ...
            try
            {
                ...
                if (resp.status == "SUCCESS")
                {
                    ...
                    {
                        return resp.GatewayPageURL ?? throw new Exception("GatewayPageURL is null.");
                    }
                }
                else
                {
                    throw new Exception("SSLCommerz Error: " + resp.failedreason);
                }
            }
            catch (Exception e)
            {
                throw new Exception("SSLCommerz Exception: " + e.Message + " | Response: " + response);
            }
